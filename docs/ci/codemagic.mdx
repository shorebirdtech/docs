---
sidebar_position: 2
title: 🚀 Codemagic
description: Integrate Shorebird into your Codemagic workflow
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Codemagic Workflow Integration

This guide will help you integrate Shorebird into your Codemagic Workflow using the Codemagic Workflow Editor and YAML configuration.

## Prerequisites

✅ Shorebird CLI is installed on your machine

✅ You are logged into a paid account.

:::info
Refer to the [getting started](/) instructions for more information.
:::

## Authentication

Most Shorebird functionality, like creating releases and patches, requires being authenticated. In order to authenticate with Shorebird in CI, you will need to generate a CI token.

```sh
shorebird login:ci
```

You will be prompted to go through a similar OAuth Flow as when using `shorebird login`, however, `shorebird login:ci` will not store any credentials on your device. Instead, a Shorebird token will be generated for you to use in CI.

The output should look something like:

```sh
$ shorebird login:ci
The Shorebird CLI needs your authorization to manage apps, releases, and patches on your behalf.

In a browser, visit this URL to log in:

https://accounts.google.com/o/oauth2/v2/auth...

Waiting for your authorization...

🎉 Success! Use the following token to login on a CI server:

<SHOREBIRD_TOKEN>

Example:

export SHOREBIRD_TOKEN="$SHOREBIRD_TOKEN" && shorebird patch android
```

:::caution
The `SHOREBIRD_TOKEN` is a secret and should not be committed directly in your source code or shared publicly.
:::

Next, copy the generated `SHOREBIRD_TOKEN` and navigate to your Codemagic secrets via:

1. Go to "Environment Variables"
2. Enter as variable name `SHOREBIRD_TOKEN`
3. Paste the token into the variable value field
4. Select or create new group, like "shorebird"
5. Make sure "Secure" is checked

It's recommended to use the variable name `SHOREBIRD_TOKEN` because it's the
default name used by the Shorebird CLI. Otherwise, you will need to care about
setting the `SHOREBIRD_TOKEN` environment variable in your Codemagic workflow.

![Screenshot of the "Environment Variables" tab in Codemagic](https://github.com/shorebirdtech/docs/assets/24459435/a2f1ee39-686b-4473-8f3e-d720e835824f)

## Codemagic Workflow Editor

_If you are using the `codemagic.yaml`, please follow the instructions in [Codemagic YAML](#codemagic-yaml)._

The docs for the Workflow Editor are coming soon. Please refer to the [Codemagic YAML](#codemagic-yaml) instructions in the meantime.

## Codemagic YAML

_If you are using Workflow Editor, please follow the instructions in [Codemagic Workflow Editor](#codemagic-workflow-editor)._

In the `codemagic.yaml` file, specify the `SHOREBIRD_TOKEN` under `environment`.

<Tabs>
  <TabItem value="maclinux" label="Mac/Linux" default>

1. Add the group "shorebird" to your environment groups. If you use a different group name, use that instead.

```yaml
workflows:
  example:
    name: Example
    environment:
      groups:
        # Exports the SHOREBIRD_TOKEN environment variable
        - shorebird
```

2. Add a script to your workflow to set up Shorebird. This script will install Shorebird and add it to your PATH.

```yaml
scripts:
  - name: 🐦 Setup Shorebird
    script: |
      # Install Shorebird
      curl --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/shorebirdtech/install/main/install.sh -sSf | bash

      # Add Shorebird to PATH
      echo PATH="/Users/builder/.shorebird/bin:$PATH" >> $CM_ENV
```

3. Execute Shorebird commands in your workflow.

```yaml
scripts:
  - name: 🚀 Shorebird Patch
    script: shorebird patch android --force
```

### Full Example

Here is an example of a full `codemagic.yaml` file:
```yaml
workflows:
  example:
    name: Example
    environment:
      groups:
        - shorebird
      flutter: stable
    scripts:
      - name: 🐦 Setup Shorebird
        script: |
          # Install Shorebird
          curl --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/shorebirdtech/install/main/install.sh -sSf | bash

          # Add Shorebird to PATH
          echo PATH="/Users/builder/.shorebird/bin:$PATH" >> $CM_ENV
      - name: 🚀 Shorebird Patch
        script: shorebird patch android --force
```

You can find here a repository with a working example: [codemagic-shorebird-demo](https://github.com/nilsreichardt/codemagic-shorebird-demo)

  </TabItem>
  <TabItem value="windows" label="Windows" default>
The docs for Windows are coming soon.

  </TabItem>
</Tabs>

### Advanced Example

If you want to specify the type (patch or release) from the outside, you can use
environment variables in your workflow.

```yaml
workflows:
  advanced-example:
    name: Advanced Example
    environment:
      vars:
        TYPE: "patch" # patch or release
      groups:
        # Exports the SHOREBIRD_TOKEN environment variable
        - shorebird
      flutter: stable
    scripts:
      - name: 🐦 Setup Shorebird
        script: |
          # Install Shorebird
          curl --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/shorebirdtech/install/main/install.sh -sSf | bash

          # Add Shorebird to PATH
          echo PATH="/Users/builder/.shorebird/bin:$PATH" >> $CM_ENV
      - name: 🙌 Shorebird Build
        script: |
          echo "➡️ Using type: $TYPE"

          # If type is not patch or release, exit with error
          if [ "$TYPE" != "patch" ] && [ "$TYPE" != "release" ]; then
            echo "TYPE must be either patch or release"
            exit 1
          fi

          # If type is patch, run patch command
          if [ "$TYPE" == "patch" ]; then
            echo "🩹 Running patch command"
            shorebird patch android --force
          fi

          # If type is release, run release command
          if [ "$TYPE" == "release" ]; then
            echo "🚀 Running release command"
            shorebird release android --force
          fi
```

Now you can execute the workflow, like:

```sh
curl --location 'https://api.codemagic.io/builds' \
--header 'x-auth-token: YOUR-AUTH-TOKEN' \
--header 'Content-Type: application/json' \
--data '{
    "appId": "YOUR-APP-ID",
    "workflowId": "advanced-example",
    "branch": "main",
    "environment": {
        "variables": {
            "TYPE": "release"
        }
    }
}'
```

This will run the release command. If you want to run the patch command, change
the `TYPE` variable to `patch`.

To get your `x-auth-token`, please follow the [Codemagic API authentication docs](https://docs.codemagic.io/rest-api/codemagic-rest-api/#authentication).