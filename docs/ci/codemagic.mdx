...
:::note
You need to specify the ios build machine in order to build ios releases and patches.
:::
...
workflows:
  example:
    ...
    environment:
      ios_signing:
        distribution_type: ad_hoc #app_store # or | development | enterprise
...
workflows:
  example:
    ...
    environment:
      ...
      ios_signing:
        distribution_type: ad_hoc #app_store # or: ad_hoc | development | enterprise
        bundle_identifier: your package name # for wildcard provisional profiles
      flutter: stable
    scripts:
      - name: 🐦 Setup Shorebird
