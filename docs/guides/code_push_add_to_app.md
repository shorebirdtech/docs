...
```sh
shorebird release android_archive --build-number 1.0 --release-version 1.2.3+1
```
...
*with the same `aar` generated by the release command above*.
...
```sh
shorebird patch android_archive --build-number 1.0 --release-version 1.2.3+1
```
...
Now run the app directly from the emulator or device (*not* from Android
