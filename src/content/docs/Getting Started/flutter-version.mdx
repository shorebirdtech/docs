...
:::tip Another way to see the current Flutter version is by running
`shorebird --version`. :::
...
:::note Only the release command supports the `--flutter-version` option.
Patches will always be built with the version of Flutter used by the release.
:::
