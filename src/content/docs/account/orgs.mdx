...
An organization's plan is determined by its **owner's** subscription. The owner
is the account that created the organization, and their Shorebird subscription
is what sets the plan (and available features) for the entire organization.

Key details:

- **One owner per organization** — each organization has a single owner whose
  subscription determines the organization's plan.
- **One account can own multiple organizations** — there is no limit to the
  number of organizations a single account can own. Each organization uses the
  owner's plan.
- **Unlimited apps per organization** — there is no limit to the number of apps
  an organization can contain.
- **Members don't need their own paid plan** — organization members get access
  to the features enabled by the owner's plan. Only the owner needs a paid
  subscription.
