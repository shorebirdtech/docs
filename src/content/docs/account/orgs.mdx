...
Organizations are a way to share groups of apps with people. This is useful for
...
See [Plans & Upgrading](/account/plans/) for how.
...
To create an organization, visit the Shorebird console and click on your email
address in the bottom left corner. From there, click the "+" button next to
"Organizations".
...
can head over to the organization's settings to change its details, including
the name.
...
click the "Transfer" button besides "Transfer Ownership" section. You can then
select the organization you want to transfer the app to from the dropdown. You
can only transfer to an organization you are part of.
...
You can also transfer an app from the command line with
[`shorebird apps transfer`](/account/cli#transfer-an-app).
...
<ArcadeEmbed
  src="https://app.arcade.software/share/Cl6LTDv4A3TKhJJOdJse"
  title="Adding Members to the Organization"
/>

To add one or more members to your organization, visit the organization's
settings, go to "Members" section and click the "Invite Member" button.
...
the plan and available features for the entire organization.

To manage billing, the organization owner can visit the
...
3. Enter the email address of the person you would like to add as a
   collaborator. Note that the email address must be associated with an existing
   Shorebird account.
