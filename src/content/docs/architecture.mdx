...
The `shorebird_cli` commands are documented on this site. Most of the build
logic is just wrapping the `flutter` tool and it also adds a few commands to
interface with Shorebird's servers.

The source for the `shorebird_cli` tool is available on GitHub:
...
The `dart-lang/sdk` repo contains the Dart SDK, including the VM, dart2js, core
libraries, and more.
