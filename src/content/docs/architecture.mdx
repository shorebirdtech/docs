...
On iOS it's a bit more complicated. Logically it works the same way (a "patch"
is simply replacing _all_ of the Dart code in your app), but technically it
works very differently than it does on other platforms. Apple has restrictions
(both policy and technical) about the mechanism through which iOS applications
are allowed to update. Shorebird is designed to let you comply with those
restrictions through use of a novel Dart interpreter.

On iOS, instead of compiling to the normal machine code, we instead compiled to
a modified format that can be then interpreted on device. Because interpreters
are slower than running compiled code directly on a CPU, we also have built
additional systems into Shorebird to minimize the use of our interpreter.

Typically even if you were to replace your entire application with a new one via
a patch (don't do that, it would be deceptive to users and thus against store
policies and thus against Shorebird policies) we would still expect to run the
vast majority (e.g. 80%) of your patched Dart code using the exiting compiled
and signed Dart code in your store-distributed application, rather than
interpreting the patch. This is because the vast majority of a typical Flutter
application is actually Flutter Framework code rather than application code.
...
These two changes combined make it so that you should expect on average 90%
(typically 98% or higher with occasional outliers) of your code in any patch to
run out of the "previous" binary included in the IPA, and thus run full-speed on
the CPU.

See also articles on our blog explaining [How Code Push Works](https://shorebird.dev/blog/how-we-built-code-push/).
