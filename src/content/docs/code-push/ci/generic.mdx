...
for input: a step that would normally prompt fails with an error that names the
flag to pass instead. For example, `shorebird patch` fails on native code
changes unless you pass `--allow-native-diffs`.
