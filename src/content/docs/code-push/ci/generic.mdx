...
Now that you have a token, it is important to keep that token secure. Using a
secrets manager for the token, or secure environment variables, is recommended,
depending on your CI setup. `shorebird` commands will look for the token to be
in a `SHOREBIRD_TOKEN` environment variable.
...
When integrating Shorebird into a continuous integration pipeline, following
these best practices is recommended to ensure reliable and efficient builds:
...
The Shorebird CLI detects CI environments, for example when the `CI` environment
variable is set, and runs in non-interactive mode. In this mode it never waits
for input. Some prompts use a default instead, and others fail with an error
that names the flag to pass. For example, `shorebird patch` fails on native code
changes unless you pass `--allow-native-diffs`, and without `--release-version`
it builds first to work out which release to patch.

Set the `SHOREBIRD_TOKEN` environment variable so the CLI never needs to log in
interactively. See [Scripting the Shorebird CLI](/code-push/ci/scripting) for
the full rules and for JSON output.

Thank you for reading this guide. If you have any questions or suggestions, feel
free to reach out on [Discord](https://discord.gg/shorebird).
