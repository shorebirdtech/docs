...
:::note You do not need to pass `--flutter-version` to `shorebird patch`.
Shorebird automatically detects the exact Flutter version used to build the
original release and uses it to compile the patch. :::
...
:::warning Releases built with `--no-codesign` cannot be launched via
`shorebird preview` or installed directly on devices. You must manually codesign
the generated `.xcarchive` in Xcode or through your signing pipeline before
distribution. :::
