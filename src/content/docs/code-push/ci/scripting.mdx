...
- **Prompts fail instead of waiting.** A command that needs an answer exits with
  ...
  `--allow-native-diffs`, instead of asking whether to continue.
...
To keep a job from failing on a prompt, pass every input as a flag: the app with
`--app-id` or `--flavor`, the release with `--release-version`, and so on.
Authenticate with an [API key](/account/api-keys) in the `SHOREBIRD_TOKEN`
environment variable, since `shorebird login` needs a browser.
...
Commands that delete something permanently, `shorebird apps delete` and
