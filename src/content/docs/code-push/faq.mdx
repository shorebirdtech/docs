...
We use the term "release" to mean preparing a binary for the stores. In
order to later generate a patch Shorebird needs to know the exact binary that
was shipped to the stores. The `shorebird release` command is used to
prepare a binary for the stores which includes the Shorebird updater.
...
restriction for updates on iOS. So as long as your application is not engaging in
...
Unfortunately, not all of these mechanisms allow 3rd parties to detect
...
[account](https://console.shorebird.dev/account) and click "Manage Plan" -> "Edit" -> "Cancel
Subscription".
