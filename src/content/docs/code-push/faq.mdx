...
2. Apply the patch with a real process restart: Settings → Apps → Force stop,
   stop the foreground service and then swipe the app away, or wait until the
   OS kills the process.

Recreating the Flutter engine without killing the process is not a supported
way to apply a patch. See [Update Strategies](/code-push/update-strategies).
