...
You can determine if a package has native code by looking at its source code
repository. If it contains folders like `android/`, `ios/`, `macos/`, or
`windows/`, it uses native code. You can also check its `pubspec.yaml` for a
`plugin` section under `flutter`.
