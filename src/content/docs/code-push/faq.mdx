...
Still have a question about Shorebird Code Push that our docs didn't cover?
...
We use the term "release" to mean preparing a binary for the stores. In order to
later generate a patch Shorebird needs to know the exact binary that was shipped
to the stores. The `shorebird release` command is used to prepare a binary for
the stores which includes the Shorebird updater.
...
We explain more of these terms in [Overview](/code-push).
...
`SHOREBIRD_TOKEN`. See our [CI setup guide](/code-push/ci/generic/) for detailed
instructions.
...
For specific versions by platform, please consult our
...
2. Run `shorebird release [platform]` (which will build the new release binary
   using the upgraded Flutter version).
...
Code push services are widely used in the industry (all of the large apps we're
...
### How should I handle patches during App Store or Google Play Store review?

We recommend avoiding publishing patches containing major new features,
significant behavior changes, or controversial updates to the active release
version currently undergoing review in the store.

During the review period, store reviewers will inspect the app binary you
submitted. If a patch is active for that release version, reviewers may download
it. To ensure a smooth, risk-free review process and remain fully compliant with
guidelines:

1. **Hold patches to the target version**: Do not release patches to the version
   undergoing review unless they are critical, non-deceptive bug fixes.
2. **Use staging tracks**: If you need to test code on devices without affecting
   the version in review, publish your patches to a separate testing track
   (e.g., `shorebird patch [platform] --track=staging`). This isolates your test
   code.
3. **Promote after approval**: Once the store review is complete and the new
   version is approved and published, you can safely promote staging patches to
   the `stable` track.
...
are installed in any specific Test Track or via TestFlight. Thus, we do not have
reliable visibility into composition of these groups, and cannot reliably gate
access to Shorebird patches based on these groups. You can read a bit more about
this on Stack Overflow discussions specifically about
...
If you would like to move from a monthly to a yearly plan, please reach out to
us via [email](mailto:contact@shorebird.dev) to complete the change.
