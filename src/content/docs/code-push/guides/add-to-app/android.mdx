...
import hybridAndroidScreenshot from '~/assets/hybrid_android_screenshot.png';

This guide explains how to use Shorebird in an Android add-to-app scenario (that
is, your app embeds Flutter UI in non-Flutter UI). If you are new to this
pattern, see the [Add Flutter to an existing app](/flutter-concepts/add-to-app/)
Flutter Concepts article first.
...
This guide assumes you have already have an Android app and a Flutter module.
The Android app will be named `android_app` and the Flutter module will be named
`flutter_module`.

This guide also assumes that you have created a Shorebird account. If you have
not, see the [Code Push guide](/code-push/initialize) for instructions.
...
Even though https://storage.googleapis.com/download.flutter.io is replaced with
...
the lower left corner of Android Studio. Then select "release" from the "Active
Build Variant" dropdown.

<Image
  src={hybridAndroidScreenshot}
  alt="Android Studio build variant set to release"
/>
