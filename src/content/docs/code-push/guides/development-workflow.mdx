...
1. Preview the release in the staging environment using the
   ...
   ```
   :::note
   Here and in the GitHub Actions workflow below, we use a track named
   `staging` to preview the hotfix in the staging environment. The name of the
   track is arbitrary and can be changed to any name you prefer.
   :::
