...
1. Squash and merge after CI checks & code review. Squashing commits keeps the
   commit history clean and makes it easier to cherry-pick commits into release
   branches.
...
1. Create a new release on GitHub (e.g., v1.0.0). This tags the commit so it is
   easy to find in the future and triggers the release workflow.
...
1. Create a new "hotfix" release on GitHub (e.g., v1.0.0-hotfix.1). This tags
   the commit so it is easy to find in the future and triggers the patch
   workflow.
