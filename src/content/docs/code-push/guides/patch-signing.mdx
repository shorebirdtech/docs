...
The primary trade-off of using patch signing is key management complexity. If
your organization already uses a cloud KMS or secrets manager, integrating with
Shorebird is straightforward since we only sign a small hash. Otherwise, you
will need to create and manage your own key material.

Signature verification adds a small overhead at app launch—under 50ms in our
testing on a 5-year-old Android phone. For apps where this matters, you can set
`patch_verification: install-only` in `shorebird.yaml` to verify signatures only
