...
import rolloutGroup76Red from '~/assets/rollout_group_76_red.png';
import rolloutFirestore50pct from '~/assets/rollout_firestore_50pct.png';
import rolloutFirestore76pct from '~/assets/rollout_firestore_76pct.png';
import rolloutGroup76Purple from '~/assets/rollout_group_76_purple.png';
...
This percentage-based rollout system has three parts:
...
Do this by assigning each user a number between 1 and 100 and saving that value
to a local cache using
...
This guide uses Firebase's Cloud Firestore to create a simple key-value pairing
of release versions to rollout percentages, although any method of retrieving a
...
Now, tie this all together by using the group number and the rollout percentage
to decide whether a user should get patches in the beta track or in the stable
track.
...
Future<UpdateTrack> _updateTrack() async {
  ...
  // If no rollout percentage is set for this version, default to stable.
...
Start by creating a release for Android:
...
Now, launch the example using `shorebird preview`, which downloads the release
and runs it on a local device:

<Image
  src={rolloutGroup76Red}
  alt="Release build showing group number 76 with a red background"
/>

This screenshot shows that the device is in group 76. This means that it will
request patches on the stable track until the rollout percentage is >= 76, at
which point it will start requesting patches in the beta track.

Because a patch hasn't been created on the beta track or a rollout percentage
set yet, there isn't much to see yet.

Change the background color from `Colors.deepPurple` to `Colors.red` and create
a patch on the beta track using the following command:
...
<Image
  src={rolloutFirestore50pct}
  alt="Cloud Firestore showing 1.0.0+1 at 50% rollout"
/>
...
Now, change the rollout percentage in Firebase to 76%:

<Image
  src={rolloutFirestore76pct}
  alt="Cloud Firestore showing 1.0.0+1 at 76% rollout"
/>

Press the update button again, and you'll now see:
...
to beta, and that the patch has been downloaded.

<Image
  src={rolloutGroup76Purple}
  alt="Patched build showing group number 76 with a purple background"
/>
