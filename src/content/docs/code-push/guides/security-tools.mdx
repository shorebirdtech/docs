...
> Obfuscation (noun): _the action of making something obscure, unclear, or
> unintelligible._
...
Obfuscation support requires Flutter 3.41.2 or later. If you are using an older
version of Flutter, `shorebird release --obfuscate` will display an error asking
you to upgrade.
...
unable to create a compatible patch for your users since they will be using a
different binary than you stored with Shorebird.

Other than the "it must be during the build" restriction, few issues have been
seen with third party security tools and Shorebird.

### GuardSquare on iOS

One issue has been seen with GuardSquare on iOS with Shorebird. GuardSquare (at
...
this is an issue for your product, the Shorebird team is happy to discuss with
the GuardSquare support team if there are possible workarounds.
