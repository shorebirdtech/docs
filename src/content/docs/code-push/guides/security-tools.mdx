...
To learn more about:
* company security, see our Handbook for [Security and
  Compliance](https://handbook.shorebird.dev/compliance/).
* how Shorebird Code Push ensures only you can control updates to your App see
  [Patch Signing](/code-push/guides/patch-signing).
...
This can also be useful for hiding internal details such as upcoming product
names e.g. `connectToSecretNewProduct()` in your app.  Although "[secret
...
might not find all the ways in which your company secrets may be encoded in your
binary.

Flutter (via Dart) supports basic obfuscation out of the box.

All you have to do is pass `--obfuscate` to your `flutter build` (or `dart
compile`) command.
...
to run *during* the build of your application, not after.
...
`shorebird patch` possible.  `shorebird patch` works by also calling `flutter
build` on your newly patched code, but then comparing that output to the saved
release artifacts.

As a result of this, if you run tools *after* the build of your application and
thus send a *different* version of your application to users, Shorebird will be
