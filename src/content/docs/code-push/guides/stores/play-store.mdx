...
:::warning Ensure the uploaded App Bundle version code and version name exactly
match the release version outputted by the Shorebird CLI build step. Modifying
the version code or build number inside the Google Play Console during or after
upload will cause patch resolution to fail for that release. :::
