...
Individual patches cannot be deleted. A patch is removed only when the release
it belongs to, or the app that owns that release, is deleted.

:::danger[Deleted patches are not restorable]

Deleting a release or an app permanently deletes every patch associated with it.
Shorebird cannot restore deleted patches for you, and there is no undo. The
console shows a confirmation dialog before the deletion goes through.

To stop serving a patch without destroying anything, use
[rollback](/code-push/rollback) instead. It remotely uninstalls the patch from
end users' devices and can itself be undone.
