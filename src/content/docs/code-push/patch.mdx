...
This command shows the patch's ID, track, whether it is rolled back, any notes,
and the size of each platform and architecture artifact.
...
```sh
shorebird patches set-track --release 1.0.0+1 --patch 1 --track staging
```

### Roll back or roll forward a patch

To stop serving a patch, roll it back. To serve it again later, roll it forward:
...
See [Roll back a Patch](/code-push/rollback#roll-back-from-the-cli) for what
happens on devices and the options these commands take.
