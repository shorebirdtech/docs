...
The iOS App Store requires all app update systems (like Shorebird's) to use an
interpreter. An interpreter is a software program that can run other programs
(rather than running the program directly on hardware). Interpreters are slow.
