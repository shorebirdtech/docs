...
Because Flutter modules (.aars and .xcframeworks, created with `shorebird release
aar` and `shorebird release ios-framework`, respectively) are embedded in apps
that Shorebird does not know about, `shorebird preview` does not support them.
...
from Xcode or Android Studio *with a Release configuration*, you will see
