...
| `--obfuscate`        |              | Obfuscate Dart code. Without `--split-debug-info`, symbols are written to `build/shorebird/symbols`.                                                                 |
| `--split-debug-info` |              | Output Dart debug symbol files to the specified directory. See [Uploading Symbols](/code-push/crash-reporting/uploading-symbols#dart-symbols-for-obfuscated-builds). |
