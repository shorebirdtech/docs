...
:::note `shorebird release` wraps `flutter build` and can take any argument
...
:::note By default `shorebird release` uses the Flutter version bundled within
the shorebird installation.
...
:::warning Deleting a release will remove all associated patches and artifacts
