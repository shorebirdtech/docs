...
Gradle NDK ABI filters (for example `ndk.abiFilters`) are not enough on their
own. Shorebird still looks for every architecture in `--target-platform` (all
three by default). If Gradle drops an ABI Shorebird was asked to upload, the
release fails. Keep those filters in sync with `--target-platform`, or drop the
Gradle filters and use the flag only.
