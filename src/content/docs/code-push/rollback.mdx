...
:::danger[Emergency Quick-Rollback] If a broken patch is live in production,
roll it back immediately using one of these options:
...
Roll back requires a minimum Flutter version of 3.27.4. This is because
engine-level support to keep track of and parse rolled back patch numbers on the
user's device was introduced starting with Shorebird's release accompanying
Flutter 3.27.4.
...
of our code locations is available under
