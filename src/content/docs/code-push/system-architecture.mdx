...
Even if you were to replace all of your app code via a patch (don't do that, it would be deceptive to users and against Shorebird and store policies), we would still expect to run the
vast majority (e.g. 80%) of your patched Dart code using the exiting compiled
and signed Dart code in your store-distributed application. This is because the vast majority of a typical Flutter
