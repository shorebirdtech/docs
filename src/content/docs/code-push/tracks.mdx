...
channel that all devices subscribe to. You can create additional tracks on the
fly, simply by naming them when publishing a patch. No configuration or
registration is needed; a track is created the first time you publish to it. To
create, list, or delete tracks yourself, see
[Managing tracks from the CLI](#managing-tracks-from-the-cli).

:::note

The CLI calls tracks **channels** in some places, such as the
`shorebird channels` commands. Track and channel mean the same thing.
...
| `staging`  | Commonly used for internal / CI validation before promoting to production.                                  |
| `beta`     | Commonly used for wider pre-production testing (e.g., QA teams, opt-in testers).                            |
| _Any name_ | Custom tracks are created on demand, or with `shorebird channels create`.                                   |

:::note

Track names are arbitrary strings. `shorebird channels create` and
`shorebird patches set-track` accept names of up to 128 characters. Use whatever
naming convention makes sense for your team. `stable`, `beta`, and `staging` are
built-in names, so those tracks can't be deleted.
...
## Managing tracks from the CLI

As of Shorebird 1.6.121, the `shorebird channels` commands list, create, and
delete an app's tracks. Like other app commands, they read the app ID from
`shorebird.yaml` in the current directory. Pass `--app-id` to run them from
anywhere else, or `--flavor` to pick a flavor's app ID. All of them support the
global `--json` flag.
...
Each line shows the track's ID and name.

### Create a track

```bash
shorebird channels create --name qa
```

You don't need to create a track before publishing to it, since
`shorebird patch --track=<name>` creates it for you. Use `channels create` when
you want the track to exist before its first patch. The command fails if a track
with that name already exists.

### Delete a track

Devices on a deleted track stop receiving patches. There's no confirmation
prompt. Instead, you confirm by repeating the track's name with
`--confirm-name`, which works the same in a terminal and in CI:

```bash
shorebird channels delete --name qa --confirm-name qa
```

The built-in `stable`, `beta`, and `staging` tracks can't be deleted.
...
Using `shorebird patches set-track` from the CLI, you can assign a patch to any
track:
...
The Shorebird console and `shorebird patches promote` only target `stable`. Use
`set-track` from the CLI when you need to move a patch to a different track.
