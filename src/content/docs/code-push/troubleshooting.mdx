...
So what does this mean? In this case, it means you added a new image to your app.
The warning is because your patched Dart code could _depend_ on those new assets
...
2. A dependency/plugin produces a different output on every build. This can
   happen if the dependency includes a timestamp indicating when it was
