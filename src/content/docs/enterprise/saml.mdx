...
Shorebird accepts a signature on the response, on the assertion, or on both, and
requires at least one. Okta signs both by default; Entra ID and Google Workspace
sign only the assertion by default, and that is fine as it is. Shorebird does
not sign its authentication requests, so the application must not require signed
requests.
