...
When the Flutter framework was designed, there was a distinct goal: **[keep
things consistently fast and smooth at 60 (or even 120) frames per second.](https://docs.flutter.dev/perf)** To
pull that off, Flutter avoids using a single, heavy structure (like the DOM in
web development). Instead, it uses a team of three different "trees" working
...
You are just telling Flutter to throw away the old stale objects and create new ones from the blueprints you created.
ones.
...
and holds onto the `State` of your `StatefulWidgets`. It's the smart middleman
that decides when actual heavy lifting needs to be done.

## 3. The RenderObject Tree: The Workhorse

Finally, we have the **RenderObject Tree**. This is where the real magic
happens.
...
Element updates its notes, but the _exact same_ RenderObject is kept around;
...
It's literally just the Element saying, "Here I am in the tree! If you need to
find an ancestor widget (like a `Theme` or a `Navigator`), you can use me to
look up the tree."

Tying it all together: the code you write every day interacts directly with this
powerful Element Tree without you even realizing it!
...
- You'll understand why [`Keys`](https://api.flutter.dev/flutter/foundation/Key-class.html) are sometimes needed (they help the Element tree
  match up widgets correctly when you reorder lists).
- You'll realize why creating lots of widgets is totally fine, but stacking
  unnecessary deep widgets like [`Opacity`](https://api.flutter.dev/flutter/widgets/Opacity-class.html) or [`Clip`](https://api.flutter.dev/flutter/widgets/ClipRect-class.html) (which create heavy
  RenderObjects) can slow things down.
