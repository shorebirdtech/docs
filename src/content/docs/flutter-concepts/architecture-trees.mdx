...
Here's a breakdown of what each tree does and how they team up to paint pixels
on your screen.
...
- If the new widget looks like the old one (same type and key), the Element just
  says "close enough," updates its settings, and stays right where it is.
...
Finally, there's the **RenderObject Tree**.
...
Here's what happens when you tap a button that changes a color:
...
It's just the Element marking its spot in the tree, so that if you need to find
an ancestor widget (like a `Theme` or a `Navigator`), you can use it to look up
the tree.
