...
import patches from '~/assets/rollback_patch2_active.png';
import rollback from '~/assets/rollback.png';
import flutterBeginnersInitialApp from '~/assets/flutter_beginners_initial_app.jpg';
import flutterBeginnersUpdatedApp from '~/assets/flutter_beginners_updated_app.jpg';
...
[The `app_id` is not secret](https://docs.shorebird.dev/code-push/faq/#do-you-need-to-keep-your-app_id-secret)
...
<Image src={flutterBeginnersInitialApp} alt="Initial app UI on Android" />

Before shipping anything, make a small but visible change so you can see how
Flutter’s UI responds to code edits. This next step tweaks the app’s theme color
and updates some on-screen text.
...
<Image src={flutterBeginnersUpdatedApp} alt="Updated app UI on Android" />
...
The next step takes this simple customization further by preparing the app for
its first Shorebird-powered release, setting the stage for OTA updates that go
...
app stores (see
[Releasing a Flutter App](/flutter-concepts/releasing-flutter-apps/)). For
Android, you get an `.aab` file for
[Google Play](/flutter-concepts/releasing-flutter-apps/android/); for iOS, an
`.ipa` for [App Store Connect](/flutter-concepts/releasing-flutter-apps/ios/).
...
![Releasing a patch](/assets/flutter_beginners_patch.gif)
