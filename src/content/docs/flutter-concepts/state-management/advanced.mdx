...
The [Beginner Guide](/flutter-concepts/state-management/beginner) explored the
core philosophy of BLoC and how to leverage the Single State + Enum pattern to
avoid screen flickering and preserve data.

Now, put these professional state management practices into action by building a
fully-featured, production-ready weather app from scratch using BLoC, the
repository pattern, and HTTP integrations.
...
Next, define the events. Events should name actions that happened.
...
during refreshes), use a Single State Class with a status enum.
...
Now write the business logic orchestrator, using `copyWith` to emit state
modifications. Notice that during refreshes, the existing weather data is
preserved.
...
This feature layout is split into:
