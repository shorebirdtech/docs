...
Sometimes when running `shorebird` commands that involve gradle processes you might face
this error: `Unsupported class file major version 65`, (or 66, the number varies sometimes).

This error usually happens because gradle is being executed with an incompatible Java version.
...
Refer to the [Gradle's official documentation](https://docs.gradle.org/current/userguide/compatibility.html)
to check which Java version works with which Gradle release.

The most safe way to set a specific java installation for shorebird to use, is to set the `JAVA_HOME`
environment variable.

To manage multiple Java installations, or to install a specific one, we recommend using
