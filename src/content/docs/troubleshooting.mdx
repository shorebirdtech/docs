...
## Can't preview my app because it is not side loaded

Operational systems sometimes have some limitations that will not allow an application to be
installed directly on the device and if an release is created with the build context that would
hit those limitations, Shorebird will not be able to preview the release in a device.

An example are iOS releases created without code signing, which are forbidden of being installed
by iOS.
